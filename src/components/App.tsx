...
export default function App() {
  ...
  const [walletAddr, setWalletAddr] = useState("0x71C...3a4F");
  const [checkedIn, setCheckedIn] = useState(false);
  const [redeemAmt, setRedeemAmt] = useState("");

  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(""), 2800); };

  const fakeTx = (cb: () => void) => {
    setTxPending(true);
    setTimeout(() => { setTxPending(false); cb(); }, 2000);
  };

  const claimFreeSpins = () => {
    if (dailySpinsClaimed) { showToast("Already claimed today! Come back tomorrow."); return; }
    fakeTx(() => {
      setFreeSpins(f => f + 2);
  ...
  const doSpin = (type: "free" | "paid") => {
    if (spinning) return;
    if (type === "free" && freeSpins <= 0) { showToast("No free spins! Claim daily or pay to spin."); return; }
    const runSpin = () => {
      setSpinning(true);
      setSpinResult(null);
      ...
      setRot(newRot);
      if (type === "free") setFreeSpins(f => f - 1);
      setTimeout(() => {
  ...
  const buildWheel = () => {
    ...
    return PRIZES.map((p, i) => {
      ...
      const mid = s(startAngle + seg / 2);
      const tx = cx + (r * 0.65) * Math.cos((startAngle + seg / 2) * Math.PI / 180);
      const ty = cy + (r * 0.65) * Math.sin((startAngle + seg / 2) * Math.PI / 180);
      return (
        <g key={i}>
          <path d={"M " + cx + " " + cy + " L " + s1.x + " " + s1.y + " A " + r + " " + r + " 0 0 1 " + s2.x + " " + s2.y + " Z"} fill={p.bg} stroke="#0d0d12" strokeWidth="2"/>
  ...
  return (
    <div style={{ background: "#0d0d12", minHeight: "100vh", maxWidth: 430, margin: "0 auto", fontFamily: "sans-serif", color: "#fff", display: "flex", flexDirection: "column", position: "relative" }}>
      {txPending && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 999, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
          ...
          <div style={{ fontSize: 14, color: "rgba(255,255,255,0.7)" }}>Confirming transaction...</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>Please wait</div>
        </div>
      )}
        ...
        {tab === "home" && (<>
          <div style={{ margin: "14px 14px 0", background: "linear-gradient(135deg,#4c1d95,#6d28d9)", borderRadius: 20, padding: 18, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
                <div style={{ fontSize: 16, fontWeight: 600 }}>{freeSpins}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>Free spins</div>
              </div>
              <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
                <div style={{ fontSize: 16, fontWeight: 600 }}>${usdc.toFixed(2)}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>USDC won</div>
              </div>
              <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
                <div style={{ fontSize: 16, fontWeight: 600 }}>{doneMissions.length}/4</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>Missions</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ margin: "10px 14px 0", background: "#13131a", borderRadius: 16, padding: 14, border: "0.5px solid rgba(255,255,255,0.07)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 600 }}>Your Profile</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2, fontFamily: "monospace" }}>{walletAddr}</div></div>
              <div style={{ fontSize: 11, color: "#4ade80", background: "rgba(74,222,128,0.1)", borderRadius: 20, padding: "3px 10px", border: "0.5px solid rgba(74,222,128,0.25)" }}>● Connected</div>
            </div>
          ...
          <div onClick={doCheckin} style={{ margin: "10px 14px 0", background: checkedIn ? "#111" : "#1a1025", border: "1px solid " + (checkedIn ? "rgba(255,255,255,0.06)" : "rgba(139,92,246,0.35)"), borderRadius: 16, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: checkedIn ? "default" : "pointer" }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}>
                {checkedIn ? "✓" : "📅"} Daily check-in
                {checkedIn && <span style={{ fontSize: 11, color: "#4ade80", background: "rgba(74,222,128,0.1)", borderRadius: 20, padding: "2px 8px" }}>Done</span>}
              </div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 3 }}>+10 pts every day · resets midnight</div>
            </div>
            {!checkedIn && <button style={{ background: "linear-gradient(135deg,#7c3aed,#6d28d9)", border: "none", borderRadius: 10, padding: "8px 16px", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>Check in</button>}
          </div>

          <div style={{ padding: "14px 14px 0" }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", letterSpacing: "0.08em", marginBottom: 10, fontWeight: 600 }}>TODAY'S MISSIONS</div>
            {missions.slice(0, 2).map(m => (
              <div key={m.id} onClick={() => doMission(m.id, m.pts)} style={{ background: "#13131a", borderRadius: 14, border: "0.5px solid rgba(255,255,255,0.06)", padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, marginBottom: 8, cursor: doneMissions.includes(m.id) ? "default" : "pointer", opacity: doneMissions.includes(m.id) ? 0.5 : 1 }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, padding: "4px 11px", borderRadius: 20, background: doneMissions.includes(m.id) ? "rgba(74,222,128,0.1)" : "rgba(139,92,246,0.15)", color: doneMissions.includes(m.id) ? "#4ade80" : "#c4b5fd", border: "0.5px solid " + (doneMissions.includes(m.id) ? "rgba(74,222,128,0.3)" : "rgba(139,92,246,0.3)") }}>{doneMissions.includes(m.id) ? "✓ Done" : "+" + m.pts + " pts"}</div>
              </div>
            ))}
          </div>
        </>)}
        ...
        {tab === "spin" && (
          <div style={{ padding: "14px 14px 0" }}>
            <div style={{ background: "#13131a", borderRadius: 20, padding: "20px 14px", border: "0.5px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", letterSpacing: -0.3 }}>🎰 Spin to Win</div>
              <div style={{ position: "relative", width: 260, height: 260 }}>
                <div style={{ position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", zIndex: 10, filter: "drop-shadow(0 2px 6px rgba(0,0,0,0.5))" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, width: "100%" }}>
                <div style={{ background: "#1a1a24", borderRadius: 14, padding: 14, border: "0.5px solid rgba(139,92,246,0.2)" }}>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 4 }}>Daily limit: 2 spins</div>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 10 }}>Claim cost: 0.00002 ETH gas</div>
                ...
                <div style={{ background: "#1a1a24", borderRadius: 14, padding: 14, border: "0.5px solid rgba(20,184,166,0.2)" }}>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 4 }}>Unlimited spins</div>
                  ...
                  <button onClick={() => doSpin("paid")} disabled={spinning} style={{ width: "100%", background: spinning ? "rgba(255,255,255,0.03)" : "linear-gradient(135deg,#0f766e,#14b8a6)", border: "none", borderRadius: 10, padding: "9px 0", fontSize: 12, color: spinning ? "rgba(255,255,255,0.3)" : "#fff", fontWeight: 600, cursor: spinning ? "default" : "pointer" }}>{spinning ? "Spinning..." : "Pay & Spin"}</button>
                </div>
              </div>
        ...
        {tab === "missions" && (
          <div style={{ padding: "14px 14px 0" }}>
            <div style={{ background: "#13131a", borderRadius: 14, padding: "12px 14px", border: "0.5px solid rgba(255,255,255,0.07)", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600 }}>Daily progress</div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{doneMissions.length} of 4 missions done</div>
              </div>
            ...
            {missions.map(m => (
              <div key={m.id} onClick={() => doMission(m.id, m.pts)} style={{ background: "#13131a", borderRadius: 14, border: "0.5px solid rgba(255,255,255,0.06)", padding: "14px", display: "flex", alignItems: "center", gap: 12, marginBottom: 8, cursor: doneMissions.includes(m.id) ? "default" : "pointer", opacity: doneMissions.includes(m.id) ? 0.55 : 1 }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 20, background: doneMissions.includes(m.id) ? "rgba(74,222,128,0.1)" : "rgba(139,92,246,0.15)", color: doneMissions.includes(m.id) ? "#4ade80" : "#c4b5fd", border: "0.5px solid " + (doneMissions.includes(m.id) ? "rgba(74,222,128,0.3)" : "rgba(139,92,246,0.3)") }}>{doneMissions.includes(m.id) ? "✓ Done" : "+" + m.pts + " pts"}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === "invite" && (
          <div style={{ padding: "14px 14px 0" }}>
            <div style={{ background: "#13131a", borderRadius: 16, padding: 20, border: "0.5px solid rgba(255,255,255,0.07)", textAlign: "center" }}>
              <div style={{ width: 60, height: 60, borderRadius: "50%", background: "linear-gradient(135deg,rgba(139,92,246,0.2),rgba(109,40,217,0.2))", border: "0.5px solid rgba(139,92,246,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", fontSize: 28 }}>👥</div>
              <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 6 }}>Invite and earn</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", lineHeight: 1.7, marginBottom: 18 }}>Share your link. When a friend joins and completes their first mission, you both earn bonus points instantly.</div>
              <div style={{ background: "#1a1a24", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10, marginBottom: 12, textAlign: "left" }}>
                ...
                <button onClick={() => showToast("Link copied!")} style={{ background: "#7c3aed", border: "none", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#fff", cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}>Copy</button>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}>
              {[{ val: "+50 pts", lbl: "You earn per referral", icon: "🪙" }, { val: "+25 pts", lbl: "Friend earns on signup", icon: "🎁" }].map(r => (
                <div key={r.val} style={{ background: "#13131a", borderRadius: 14, padding: 16, border: "0.5px solid rgba(255,255,255,0.07)", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 4, lineHeight: 1.4 }}>{r.lbl}</div>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 10, background: "#13131a", borderRadius: 14, border: "0.5px solid rgba(255,255,255,0.07)", overflow: "hidden" }}>
              {[{ label: "Friends invited", val: "0" }, { label: "Pending joins", val: "0" }, { label: "Bonus pts earned", val: "0 pts" }].map((r, i, arr) => (
                <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 14px", borderBottom: i < arr.length - 1 ? "0.5px solid rgba(255,255,255,0.06)" : "none" }}>
        ...
        {tab === "wallet" && (
          <div style={{ padding: "14px 14px 0" }}>
            <div style={{ background: "linear-gradient(135deg,#1a1025,#13131a)", borderRadius: 20, padding: 18, border: "1px solid rgba(139,92,246,0.2)", marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 4, letterSpacing: "0.06em" }}>CONNECTED WALLET</div>
              <div style={{ fontSize: 14, fontFamily: "monospace", color: "#a78bfa", marginBottom: 14 }}>{walletAddr}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: "12px 10px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>Points</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: "#a78bfa" }}>{pts}</div>
                </div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: "12px 10px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>USDC won</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: "#2dd4bf" }}>${usdc.toFixed(2)}</div>
                </div>
              </div>
            </div>

            <div style={{ background: "#13131a", borderRadius: 16, padding: 16, border: "0.5px solid rgba(255,255,255,0.07)", marginBottom: 10 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>💸 Claim USDC Reward</div>
              ...
              <button onClick={() => { if (usdc <= 0) { showToast("No USDC to claim!"); return; } fakeTx(() => { setUsdc(0); showToast("✓ USDC sent to your wallet!"); }); }} style={{ width: "100%", background: usdc > 0 ? "linear-gradient(135deg,#0f766e,#14b8a6)" : "rgba(255,255,255,0.04)", border: "none", borderRadius: 11, padding: 12, fontSize: 13, color: usdc > 0 ? "#fff" : "rgba(255,255,255,0.2)", fontWeight: 600, cursor: usdc > 0 ? "pointer" : "default" }}>Claim to wallet</button>
            </div>
            ...
            <div style={{ background: "#13131a", borderRadius: 16, padding: 16, border: "0.5px solid rgba(255,255,255,0.07)" }}>
              ...
              {[{ label: "Spin reward", val: "+10 pts", time: "2m ago", color: "#a78bfa" }, { label: "Mission done", val: "+20 pts", time: "1h ago", color: "#a78bfa" }, { label: "Spin reward", val: "+0.02 USDC", time: "3h ago", color: "#2dd4bf" }, { label: "Daily check-in", val: "+10 pts", time: "1d ago", color: "#a78bfa" }].map((tx, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: i < 3 ? "0.5px solid rgba(255,255,255,0.05)" : "none" }}>
      ...
      {toast && <div style={{ position: "fixed", bottom: 90, left: "50%", transform: "translateX(-50%)", background: "rgba(17,17,25,0.95)", border: "0.5px solid rgba(139,92,246,0.3)", color: "#fff", padding: "10px 20px", borderRadius: 20, fontSize: 13, fontWeight: 500, whiteSpace: "nowrap", zIndex: 200, backdropFilter: "blur(10px)" }}>{toast}</div>}
    </div>
  );
}
